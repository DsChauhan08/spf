...
void spf_secconfig_generate_example(const char* path) {
    ...
    fprintf(fp, "# SPF Security Configuration\n");
    fprintf(fp, "# Generated by spf --generate-config\n\n");
    
    fprintf(fp, "# ==== Rate Limiting ====\n");
    fprintf(fp, "# Maximum requests allowed per time window\n");
    fprintf(fp, "rate_limit_requests = 100\n");
    fprintf(fp, "# Time window in seconds\n");
    fprintf(fp, "rate_limit_window = 60\n");
    fprintf(fp, "# Maximum concurrent connections per IP\n");
    fprintf(fp, "max_connections_per_ip = 10\n");
    fprintf(fp, "# Maximum total concurrent connections\n");
    fprintf(fp, "max_total_connections = 1000\n\n");
    
    fprintf(fp, "# ==== Attack Prevention ====\n");
    fprintf(fp, "# Enable/disable specific security checks\n");
    fprintf(fp, "sql_injection_check = true\n");
    fprintf(fp, "xss_check = true\n");
    fprintf(fp, "command_injection_check = true\n");
    fprintf(fp, "path_traversal_check = true\n");
    fprintf(fp, "slow_loris_protection = true\n");
    fprintf(fp, "user_agent_check = true\n\n");
    
    fprintf(fp, "# ==== TLS Settings ====\n");
    fprintf(fp, "# Minimum TLS version (1.2 or 1.3)\n");
    fprintf(fp, "tls_min_version = 1.2\n");
    fprintf(fp, "# Require client certificates (mTLS)\n");
    fprintf(fp, "tls_require_client_cert = false\n\n");
    
    fprintf(fp, "# ==== IP Filtering ====\n");
    fprintf(fp, "# Comma-separated list of IPs that bypass all checks\n");
    fprintf(fp, "#ip_whitelist = 192.168.1.1,10.0.0.1\n");
    fprintf(fp, "# Comma-separated list of blocked IPs\n");
    fprintf(fp, "#ip_blacklist = 1.2.3.4,5.6.7.8\n");
    fprintf(fp, "# Enable geo-blocking\n");
    fprintf(fp, "geo_blocking = false\n");
    fprintf(fp, "# Blocked country codes (ISO 3166-1 alpha-2)\n");
    fprintf(fp, "#geo_blocked_countries = CN,RU,KP\n\n");
    
    fprintf(fp, "# ==== Request Limits ====\n");
    fprintf(fp, "# Maximum request body size in bytes\n");
    fprintf(fp, "max_request_size = 10485760\n");
    fprintf(fp, "# Maximum headers size in bytes\n");
    fprintf(fp, "max_header_size = 16384\n");
    fprintf(fp, "# Maximum URI length\n");
    fprintf(fp, "max_uri_length = 8192\n");
    fprintf(fp, "# Request timeout in seconds\n");
    fprintf(fp, "request_timeout = 30\n\n");
    
    fprintf(fp, "# ==== Logging ====\n");
    fprintf(fp, "log_blocked_requests = true\n");
    fprintf(fp, "log_suspicious_activity = true\n");
    fprintf(fp, "# Hash IPs in logs for privacy\n");
    fprintf(fp, "anonymize_logs = false\n\n");
    
    fprintf(fp, "# ==== Custom Rules ====\n");
    fprintf(fp, "# Add your own patterns to block (comma-separated)\n");
    fprintf(fp, "#custom_block_patterns = \\.bak$,\\.old$,~$\n");
    fprintf(fp, "# Paths to skip security checks (comma-separated)\n");
    fprintf(fp, "#custom_allow_paths = /health,/status,/metrics\n\n");
    
    fprintf(fp, "# ==== Advanced ====\n");
    fprintf(fp, "# Anomaly score threshold (0-100, lower = stricter)\n");
    fprintf(fp, "anomaly_threshold = 80\n");
    fprintf(fp, "# Enable honeypot endpoints\n");
    fprintf(fp, "enable_honeypot = false\n");
    fprintf(fp, "# Automatically block IPs after repeated violations\n");
    fprintf(fp, "auto_block = true\n");
    fprintf(fp, "# Number of violations before auto-blocking\n");
    fprintf(fp, "auto_block_threshold = 10\n");
    fprintf(fp, "# Auto-block duration in minutes\n");
    fprintf(fp, "auto_block_duration = 60\n");
    
    fclose(fp);
    spf_log(SPF_LOG_INFO, "secconfig: generated example config at %s", path);
}
